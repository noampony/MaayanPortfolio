...
/**
 * One education entry - the degree, or any further qualification alongside it. Rendered as
 * a card at the foot of the Experience timeline.
 */
export type AboutEducation = {
  dateRange: string;
  /** The qualification's name (a degree, a bootcamp, …). */
  degree: string;
  institution: string;
...
export type AboutSectionData = {
  ...
  stats: AboutStats;
  mainFields: string[];
  /** The primary qualification - the degree. */
  education: AboutEducation;
  /** Further qualifications held alongside the degree (bootcamps and the like). */
  furtherEducation?: AboutEducation[];
...
/**
 * An earlier role held at the same organization as its parent {@link Experience}.
 *
 * Lets one card represent a progression (e.g. a promotion) instead of duplicating the
 * organization across two timeline nodes: the card renders the parent role first, then
 * one separated section per entry here.
 *
 * `durationLabel` is owner-supplied - these are completed roles, so nothing is computed
 * from a clock (the card is a client component and must not depend on one).
 */
...
export type Experience = {
  ...
  technologies?: string[];
  /**
   * Earlier roles at this same organization, most-recent-first. Rendered inside this
   * card below a separator rather than as separate timeline nodes.
   */
...
export type Project = {
  ...
  workplace?: {
    name: string;
    /** Logo path served from `/public` (e.g. `/logos/check-point-dark-ink.svg`). */
    logo: AssetReference;
