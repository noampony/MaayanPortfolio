...
function assertOptionalAboutEducationList(
  value: unknown,
  path: string,
): AboutEducation[] | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (!Array.isArray(value)) {
    throw new ContentValidationError(path, "optional array of education entries");
  }
  return value.map((item, index) => validateAboutEducation(item, `${path}[${index}]`));
}
...
export function validateAboutSectionData(data: unknown): AboutSectionData {
  ...
  return omitUndefined({
    ...
    furtherEducation: assertOptionalAboutEducationList(
      raw.furtherEducation,
      `${path}.furtherEducation`,
    ),
    professionalFocus: assertOptionalString(raw.professionalFocus, `${path}.professionalFocus`),
  });
}

/** An earlier role at the same organization (spec §11.2, `Experience.previousRoles`). */
function validateExperienceRole(value: unknown, path: string): ExperienceRole {
  const raw = assertObject(value, path);
...
function assertOptionalExperienceRoles(
  value: unknown,
  path: string,
  ...
  if (!Array.isArray(value)) {
    throw new ContentValidationError(path, "optional array of previous roles");
  }
  return value.map((item, index) => validateExperienceRole(item, `${path}[${index}]`));
}
...
export function validateExperience(data: unknown): Experience {
  ...
  return omitUndefined({
    ...
    description: assertRequiredString(raw.description, `${path}.description`),
    technologies: assertOptionalStringArray(raw.technologies, `${path}.technologies`),
    previousRoles: assertOptionalExperienceRoles(raw.previousRoles, `${path}.previousRoles`),
...
export function validateProject(data: unknown): Project {
  ...
  return omitUndefined({
    ...
    customerFocus: assertOptionalString(raw.customerFocus, `${path}.customerFocus`),
