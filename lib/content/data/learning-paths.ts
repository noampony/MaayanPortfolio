...
const learningPathData = [
  ...
  {
    // Sourced from the owner's CV ("Shani Penkar - CV" → Education). Appended rather than
    // slotted into the front so the existing paths keep their numbering; reorder once the
    // full roadmap is re-planned. No cover art exists yet, so the card falls back to its
    // gradient visual (never a broken image).
    id: "web-development",
    order: 8,
    title: "Web Development",
    courses: [course("The Complete 2022 Web Development Bootcamp")],
  },
] as const;
