...
const phoneHref = `tel:${businessCard.phone.replace(/(?!^\+)[^\d]/g, "")}`;
...
export function FloatingCard() {
  ...
  /** Set when the current close came from Escape (see {@link handleCancel}). */
  ...
  useEffect(() => {
    ...
    if (dialog.open) {
      // `close()` restores focus to the trigger synchronously, so an
      // Escape-driven close can drop that focus immediately afterwards.
      dialog.close();
  ...
  /**
   * Escape close. The native `<dialog>` would restore focus to the trigger,
   * leaving a `:focus-visible` ring on a button the user never deliberately
   * returned to (owner request: Escape should just dismiss the card). Cancel the
   * browser's own close so the effect above owns it and can blur the trigger
   * right after the focus restore. Other closes keep the restore intact.
   */
  ...
  return (
    <>
      <CardTrigger ref={triggerRef} open={open} onClick={() => setOpen(true)} />
      <dialog
        ref={dialogRef}
      ...
      >
        ...
        <div className="business-card-panel">
          ...
          <ul className="business-card-contact-list">
            ...
            <li>
              <a href={phoneHref} className="business-card-contact-link">
                <PhoneIcon />
