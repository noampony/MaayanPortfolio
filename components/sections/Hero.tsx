  ...
  return (
    ...
    >
      {/* Atmospheric backdrop - tokens only (§6.3) + decorative floating code (§8.1).
         A cluster of soft pastel blobs bleeds in from the left edge over the cream
         page, the way the theme's reference composition does; a single lavender blob
         on the right balances the portrait. Pastels on a light page need far more
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute -left-24 -top-24 h-[min(60vh,26rem)] w-[min(80vw,26rem)] rounded-full bg-pastel-lavender opacity-65 blur-3xl" />
        <div className="absolute -left-40 top-1/4 h-[min(50vh,22rem)] w-[min(70vw,24rem)] rounded-full bg-pastel-pink opacity-55 blur-3xl" />
        <div className="absolute -left-20 bottom-0 h-[min(46vh,20rem)] w-[min(64vw,22rem)] rounded-full bg-pastel-peach opacity-50 blur-3xl" />
        <div className="absolute -left-10 -bottom-28 h-[min(34vh,15rem)] w-[min(44vw,16rem)] rounded-full bg-pastel-mint opacity-40 blur-3xl" />
        <div className="absolute -right-1/4 top-1/4 h-[min(70vh,28rem)] w-[min(90vw,32rem)] rounded-full bg-pastel-lavender opacity-35 blur-3xl" />
