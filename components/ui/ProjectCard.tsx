 ...
 * solved, customer focus when present, tech stack). `whyImportant` is intentionally dropped
...
export function ProjectCard({ project, headingId, backgroundImage, backgroundOpacity }: ProjectCardProps) {
  const { name, role, workplace, shortDescription, problemSolved, customerFocus, techStack } = project;
      ...
      <div className="project-flip-inner">
        ...
        <div className="project-flip-face project-flip-front" aria-hidden={flipped}>
          ...
          <div className="project-flip-front-body">
            ...
            <span className="project-flip-front-meta">
              <span className="project-role-pill">{role}</span>
              {workplace ? (
                <span className="project-role-pill project-workplace-tag">
        ...
        >
          ...
          {workplace ? (
            <span className="project-workplace-badge">
              {/* eslint-disable-next-line @next/next/no-img-element -- logo dimensions are variable SVGs; next/image requires explicit width/height */}
              <img
                src={workplace.logo}
                alt={workplace.name}
                className="project-workplace-badge-logo"
              />
            </span>
          ) : null}
          ...
          <dl className="mt-3 space-y-2">
            ...
            {customerFocus ? (
              <div>
                <dt className="text-small font-medium text-text-muted">Customer focus</dt>
                <dd className="m-0 mt-0.5 text-small text-text-secondary">{customerFocus}</dd>
              </div>
            ) : null}
          </dl>
